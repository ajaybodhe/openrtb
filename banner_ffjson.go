...

	{
		...
		if tok == fflib.FFTok_null {
			...
			for {
				...
				/* handler: tmp_uj__Format type=openrtb.Format kind=struct quoted=false*/

				{
					...
					err = json.Unmarshal(tbuf, &tmp_uj__Format)
					if err != nil {
						return fs.WrapErr(err)
					}
				}

				uj.Format = append(uj.Format, tmp_uj__Format)
